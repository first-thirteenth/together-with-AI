...
export const Quiz = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Исправлено: удалили неиспользуемый '_' перед запятой, чтобы линтер не ругался
    const formattedAnswers = Object.entries(answers)
      .map(([, item]) => `• *${item.question}*\n  └ _${item.answer}_`)
      .join('\n\n');


    // Очищаем телефон от лишних символов прямо внутри формирования строки
    const message = `
      ...
      ⚡️ *Быстрая связь:*
      • [Написать в WhatsApp](https://wa.me/${userData.phone.replace(/\D/g, '')})
    `.trim();
    ...
    try {
      ...
      console.error(error);
      alert('Произошла ошибка при отправке данных. Проверьте, запущен ли бот кнопкой СТАРТ.');
    } finally {
      setIsSending(false);
    }
  };
            ...
            <div className="min-h-[160px] space-y-3">
              ...
              <div className="space-y-2 pt-1">
                {currentQuestion?.options.map((option) => {
                  const isSelected = answers[currentQuestion.id]?.answer === option.label;
                  return (
                    <button
                      key={option.value}
                      type="button"
          ...
          >
            <div className="text-center space-y-2 mb-4">
              ...
              <p className="text-xs text-cream-bg/60">
                Мы подобрали оптимальные программы легализации. Оставьте контакты для получения чек-листа документов.
              </p>
            </div>
